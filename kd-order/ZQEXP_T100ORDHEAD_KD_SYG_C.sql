 ...
 WHERE CUST.FLGACCBKORD = '-1'
      -- Non exported orders only
   AND NVL(t.DTETOHOST, TO_DATE('30121899', 'ddmmyyyy')) =
       TO_DATE('30121899', 'ddmmyyyy')
      -- Orders not locked by user
   AND (t.DTELCK IS NULL OR t.DTELCK < SYSDATE - 1)
      -- Orders in valid status only
   AND t.CODSTATUS = '1'
   AND t.CODDIV = 'SYG'
   AND t.CODTYPORD = '0'
;
